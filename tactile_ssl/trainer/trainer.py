...
class Trainer:
    ...
    def fit(
        ...
        self.fabric.call("on_fit_start", train_loader, val_loader, self)
    ...
    def train_loop(
        ...
        for batch_idx, batch in enumerate(iterable):
            ...
            if should_optim_step:
                ...
                self.step_wd_scheduler(wd_scheduler_cfg, level="step", current_value=self.global_step)
            # add output values to progress bar
            log_dict = {"loss": self._current_train_return["loss"], "d_time": dataloading_time}

            if grad_norm is not None:
                log_dict["grad_norm"] = grad_norm
            self._format_iterable(iterable, log_dict, "train")
    ...
    def step(self) -> int:
        ...
        return self.global_step if self.training_state == "train" else self.global_val_step
    ...
    def get_latest_checkpoint(checkpoint_dir: str) -> Optional[str]:
        ...
        if not os.path.isdir(checkpoint_dir):
            log.warning("Checkpoint should be a directory containing the checkpoints to resume training")
            return None
    ...
    def _parse_optimizers_schedulers(self, configure_optim_output) -> Tuple[
        ...
        if isinstance(configure_optim_output, Mapping):
            ...
            if "scheduler" in _lr_sched_defaults:
                ...
                raise ValueError("Invalid configuration for lr scheduler or weight decay scheduler")
        ...
        if isinstance(configure_optim_output, (list, tuple)):
            ...
            # optimizer and lr scheduler
                ...
                assert len(configure_optim_output) == 3, "Invalid optimizer configuration"
    ...
    def num_training_steps(self) -> int:
        ...
        dataset_size = self.trainer.limit_train_batches if self.trainer.limit_train_batches != 0 else len(dataset)
