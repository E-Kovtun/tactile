...
class DINOv2Module(Module, nn.Module):
    def __init__(
        ...
        assert len(self.online_probes) == len(online_probes_lrs), (
        ...
        while not valid_mask:
            ...
            if not valid_mask:
                ...
                if timeout == 0:
                    tries += 1
                    timeout = og_timeout
                    log.warning(f'Mask generator says: "Valid mask not found, decreasing acceptable-regions [{tries}]"')
        mask = mask.squeeze()
        # --
    ...
    def sample_masks(self, x):
        ...
        patch_size = self.student_encoder_dict["backbone"].patch_size
        ...
        local_maskblock_sizes = self._sample_block_size(height, width, self.local_mask_scale)
        global_maskblock_sizes = self._sample_block_size(height, width, self.global_mask_scale)
        ...
        for _ in range(batch_size):
            ...
            for _ in range(self.num_local_masks):
                mask, mask_complement = self._sample_block_mask(height, width, local_maskblock_sizes)
    ...
    def forward(self, x: torch.Tensor, global_masks: torch.Tensor, local_masks: torch.Tensor, ibot_masks: torch.Tensor):
        ...
        student_global_dict = self.student_encoder_dict["backbone"].forward_features(
            x, global_masks, masktoken_masks=ibot_masks
        )
        student_local_dict = self.student_encoder_dict["backbone"].forward_features(x, local_masks)
        ...
        student_global_patch_tokens = student_global_dict["x_norm_patchtokens"].flatten(0, 1)

        student_masked_patch_tokens = student_global_patch_tokens.new_zeros(
            num_ibot_tokens, student_global_patch_tokens.shape[-1]
        )
        ...
        with torch.no_grad():
            ...
            teacher_masked_patch_tokens = teacher_global_patch_tokens.new_zeros(
                num_ibot_tokens, teacher_global_patch_tokens.shape[-1]
            )
        ...
        patch_loss = ibot_loss_scale * self.ibot_patch_loss(
            student_masked_patch_tokens_after_head, teacher_ibot_softmaxed_centered
        )
    ...
    def training_step(self, batch: Dict[str, Any], batch_idx: int) -> Dict:
        ...
        if len(self.online_probes) > 0:
            with torch.no_grad():
                ...
                x_patch_gt = patchify_image(x, self.teacher_encoder_dict["backbone"].patch_size)
        ...
        for probe in self.online_probes:
            ...
            if probe_name == "reconstruction":
                ...
                patch_size = self.teacher_encoder_dict["backbone"].patch_size
                img_size = self.teacher_encoder_dict["backbone"].img_size
                pred_img = patches_to_image(decoded_x.detach(), patch_size, img_size).float()
                output["pred_img"] = pred_img[:, 0:3, :, :]
                output["gt_img"] = x[:, 0:3, :, :]

            elif "classification" in probe_name:
                gt_labels = batch[probe_name]
                probe_loss, pred_logits = probe(embedding, target=gt_labels)
    ...
    def configure_optimizers(  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        self._reset_training_schedules(start_step=0)
    ...
    def _reset_training_schedules(self, start_step: int) -> None:
        ...
        if not 0 <= start_step <= self._schedule_total_steps:
            raise RuntimeError(
                f"Invalid training schedule step {start_step}; expected [0, {self._schedule_total_steps}]"
            )
    ...
    def load_checkpoint_state(
        ...
        if state is None:
            log.warning(
                "Legacy checkpoint has no DINO algorithm state; deriving schedule and mask position from global_step"
            )
            ...
            schedule_step = int(state["schedule_step"])
        self._reset_training_schedules(start_step=schedule_step)

    def _teacher_temp_at_step(self, step: int):
        assert isinstance(self.teacher_temp, tuple)
