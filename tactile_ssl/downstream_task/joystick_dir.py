...
class XelaJoystickSLModule(SLModule):
    ...
    def training_step(self, batch: Dict[str, Any], batch_idx: int) -> Dict:
        ...
        graph_info = self._graph_to_device(batch.get("graph"), cond.device, repeats=chunked_time)
        cond = einops.rearrange(cond, "b (l k) n c -> (b l) k n c", k=self.sequence_length)
        z = self._forward_encoder(cond, graph_info=graph_info)["x_norm_patchtokens"]  # pyright: ignore[reportCallIssue]
