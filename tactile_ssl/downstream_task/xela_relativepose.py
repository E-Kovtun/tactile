...
class XelaRelativePoseSpatialGATv2Decoder(XelaRelativePoseDecoder):
    """Relative-pose decoder with a supervised physical-graph GATv2 coordinate encoder."""
    ...
    def _prepare_tokens(self, z, spatial_coords=None, graph_info=None):
        if spatial_coords is None:
            raise ValueError("spatial_coords are required for XelaRelativePoseSpatialGATv2Decoder")
        if spatial_coords.shape[:-1] != z.shape[:-1]:
            raise ValueError(
                "spatial_coords and signal tokens must have matching batch, time, and sensor dimensions; "
                f"got {tuple(spatial_coords.shape)} and {tuple(z.shape)}"
...
class XelaRelativePoseSpatialAttentionDecoder(XelaRelativePoseDecoder):
    """Relative-pose decoder with supervised full spatial self-attention."""
